					...
					for i in objects:
						if category_index[int(objects[i])]['name'] == input:
							...
							while True:
								...
								vis_util.visualize_only_mapped_boxes_on_image_array(map_frame, idx, np.squeeze(boxes_arr), np.squeeze(classes_arr).astype(np.int32), np.squeeze(scores_arr), category_index, use_normalized_coordinates=True)
