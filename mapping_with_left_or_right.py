# python mapping_with_left_or_right.py --prototxt MobileNetSSD_deploy.prototxt.txt --model MobileNetSSD_deploy.caffemodel
						...
						while True:
							...
							for i in np.arange(0, all_detections.shape[2]):
								if index == int(all_detections[0, 0, i, 1]):
									box_dimension = all_detections[0, 0, i, 3:7] * np.array([w, h, w, h])
									(sX, sY, eX, eY) = box_dimension.astype("int")
